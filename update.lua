            ...
            elseif notify:getMainType() == 1 then--成功
                ...
                if self.updateCount == self.totalCount then--更新完毕
                    ...
                    elseif self.isRestartGame then --重启游戏 
                        GameManager:showMustAlert(cc.Director:getInstance():getRunningScene(),"更新完毕，点击重新进入游戏",function()
                            jt.CFrameData:getInstance():exitApp()
                        end)
                    end
                end
...
function update:loadVersions()
     local function onResponse(urlData)
        ...
        if urlData:getStatusCode() == URLLoaderHandler.STATUS_CODES.SUCCESS then
            ...
            if not status then
                GameManager:showMustAlert(cc.Director:getInstance():getRunningScene(),"版本资料加载失败，数据格式有误")
                self.delegate:playGame(true)
                return
            end
            ...
            for i=1,#vList do
                local item = vList[i]
                local targetV = item.version --{GameManager:getProject():getVersionList(item.version)}
                local nowV = nowVersion--{GameManager:getProject():getVersionList(nowVersion)}
                if tonumber(targetV) > tonumber(nowV) then
                    --小版本更新
                    if item.skipUpdate ~= true then
                        isNeedUpdate = true
                        print("待更新版本:",item.version)
                        if string.find(item.downloadURL,"http") == nil then
                            item.downloadURL = GameManager:getProject():getActionURL() .. item.downloadURL
--                            item.downloadURL = "http://120.24.70.142:2020"..item.downloadURL
                        end
    ...
    print("正在检查新版本，请稍等")
--    local bigV = GameManager:getProject():getVersionList(GameManager:getProject().AppVersions)
--    local vURL = "http://120.24.70.142:2020/version/"..GameManager:getProject().AppVersions.."/version.txt"
    local vURL = GameManager:getProject():getActionURL().."/version/"..GameManager:getProject().AppVersions.."/version.txt"
...
--    local vURL = "http://".. GameManager:getProject().LoginAddress .. GameManager:getProject().URLPort .. "/version/" .. GameManager:getProject().AppVersions .. "/version.txt"
    self.isLoading = true
